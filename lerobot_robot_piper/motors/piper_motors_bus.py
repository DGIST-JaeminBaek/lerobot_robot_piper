...
class PiperMotorsBus(MotorsBus):
    ...
    def get_velocity(self) -> dict[str, float]:
        # motor_speed: 0.001 rad/s. NEXT(외력 추정)가 자세/속도로 인한 effort 노이즈를
    ...
    def enter_mit_compliance(
        ...
        # ponytail: 위치 유지형 컴플라이언스(t_ref=0)만 구현 — 실제 순응 힘/kp-kd 튜닝은
        # 랩 PC 실기에서. is_overloaded() 감지 시 우선은 send_action 쪽에서 명령을
