...
>
> 내 담당 과제 두 가지 중 두 번째. 첫 번째(effort)는 `docs/effort_guide.md`.
>
> **출처 표기 원칙**
...
```
1280×720 (16:9)  →  resize_with_pad  →  512×288 + 패딩  →  512×512
 640×480  (4:3)  →  resize_with_pad  →  512×384 + 패딩  →  512×512
```

**둘 다 512×512 텐서가 된다.** 720p로 찍으면 픽셀의 상당 부분을 축소 과정에서 버린다.
...
> **판단**: SmolVLA만 쓸 거라면 **640×480이 합리적**이다. 720p는 인코딩·저장을
> 3배 쓰면서 모델 입력은 같다.
>
