...
class RealSenseCamera(Camera):
    ...
    @check_if_not_connected
    def async_read_paired(self, timeout_ms: float = 200) -> tuple[NDArray[Any], NDArray[Any] | None]:
        """color와 depth를 같은 frameset에서 원자적으로 함께 반환.

        _read_loop()는 매 반복마다 하나의 frameset에서 color/depth를 같이 꺼내
        `with self.frame_lock:` 한 블록 안에서 두 필드를 동시에 갱신한다 — 즉
        메모리상으로는 항상 짝이 맞는 상태다. 문제는 async_read()와 read_depth()를
        따로 호출하면(Piper 쪽 get_observation()처럼 두 콜을 동시에 스레드로
        돌리는 경우 특히) 둘 다 같은 new_frame_event를 기다렸다 지우면서 서로
        경쟁하게 되고, read_depth()는 내부적으로 async_read()를 한 번 더 호출해서
        이 경쟁이 더 심해진다 — 결과적으로 color와 depth가 다른 순간의 frameset에서
        나올 수 있다(짝이 안 맞음). 이 메서드는 락을 한 번만 잡고 두 필드를 같이
        스냅샷 떠서 항상 같은 frameset 쌍임을 보장한다.

        Args:
            timeout_ms: 새 프레임을 기다릴 최대 시간(ms).

        Returns:
            (color, depth) 튜플. use_depth=False면 depth는 None.
        """
        if self.thread is None or not self.thread.is_alive():
            raise RuntimeError(f"{self} read thread is not running.")
