...
def encode_video_frames(
    ...
    if extra_options:
        video_options.update({key: str(value) for key, value in extra_options.items()})
        ...
        for input_data in input_list:
            with Image.open(input_data) as input_image:
                if pix_fmt.startswith("gray"):
                    input_array = np.asarray(input_image, dtype=np.uint16)
                    input_frame = av.VideoFrame.from_ndarray(input_array, format=pix_fmt)
                else:
...
def decode_depth_video_frames(
    ...
    """Decode single-channel high-bit-depth frames without RGB conversion or 8-bit normalization."""
    loaded_frames = []
    loaded_timestamps = []
    first_ts = min(timestamps)
    last_ts = max(timestamps)
    ...
    with av.open(str(video_path), "r") as container:
        ...
        if stream.time_base is not None:
            # Seek with enough pre-roll for keyframe lookup and HEVC frame reordering.
            seek_ts = max(0.0, first_ts - 1.0)
            seek_offset = int(seek_ts / float(stream.time_base))
            container.seek(seek_offset, stream=stream, backward=True, any_frame=False)

        for frame in container.decode(stream):
            frame_ts = float(frame.time) if frame.time is not None else float(frame.pts * stream.time_base)
            loaded_frames.append(frame.to_ndarray(format=pix_fmt))
            loaded_timestamps.append(frame_ts)
            if frame_ts >= last_ts:
                break

    if not loaded_frames:
        raise RuntimeError(f"No depth frames decoded from {video_path}")

    query_ts = np.asarray(timestamps, dtype=np.float64)
    decoded_ts = np.asarray(loaded_timestamps, dtype=np.float64)
    closest_indices = np.abs(query_ts[:, None] - decoded_ts[None, :]).argmin(axis=1)
    distances = np.abs(query_ts - decoded_ts[closest_indices])
    if not np.all(distances < tolerance_s):
        raise FrameTimestampError(
            f"Depth frame timestamps exceed tolerance {tolerance_s}: {distances.tolist()} "
            f"for queries {timestamps} in {video_path}"
        )

    frames = np.stack([loaded_frames[index] for index in closest_indices])
