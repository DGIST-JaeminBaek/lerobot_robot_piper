...
def sample_images(
    image_paths: list[str], dtype: np.dtype = np.uint8, force_rgb: bool = True
) -> np.ndarray:
    ...
    for i, idx in enumerate(sampled_indices):
        ...
        img = load_image_as_numpy(path, dtype=dtype, channel_first=True, force_rgb=force_rgb)
        img = auto_downsample_height_width(img)

        if images is None:
            images = np.empty((len(sampled_indices), *img.shape), dtype=dtype)
...
def compute_episode_stats(
    ...
    for key, data in episode_data.items():
        ...
        if features[key]["dtype"] in ["image", "video"]:
            ...
            is_depth = bool(info.get("is_depth_map") or info.get("video.is_depth_map"))
            ep_ft_array = sample_images(
                data,
                dtype=np.uint16 if is_depth else np.uint8,
                force_rgb=not is_depth,
            )
            if is_depth:
                ep_ft_array = dequantize_depth(
                    ep_ft_array,
        ...
        if features[key]["dtype"] in ["image", "video"] and not is_depth:
            ep_stats[key] = {
...
def _validate_stat_value(value: np.ndarray, key: str, feature_key: str) -> None:
    ...
    if "image" in feature_key and key != "count" and value.shape not in [(1, 1, 1), (3, 1, 1)]:
        raise ValueError(
            f"Shape of quantile '{key}' must be (1,1,1) or (3,1,1), but is {value.shape} instead."
        )
