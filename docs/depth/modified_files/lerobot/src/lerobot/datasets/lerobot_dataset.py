...
def _is_depth_feature(feature: dict) -> bool:
    ...
    return bool(info.get("is_depth_map") or info.get("video.is_depth_map"))


def _depth_quantizer_kwargs(feature: dict) -> dict:
    info = feature.get("info") or {}
    return {
        "depth_min": info.get("video.depth_min", info.get("depth_min", DEFAULT_DEPTH_MIN)),
        "depth_max": info.get("video.depth_max", info.get("depth_max", DEFAULT_DEPTH_MAX)),
        "shift": info.get("video.shift", info.get("shift", DEFAULT_DEPTH_SHIFT)),
        "use_log": info.get("video.use_log", info.get("use_log", DEFAULT_DEPTH_USE_LOG)),
    }
...
class LeRobotDatasetMetadata:
    ...
    def update_video_info(self, video_key: str | None = None) -> None:
        ...
        for key in video_keys:
            ...
            existing_info = self.features[key].get("info", {})
            video_info = get_video_info(video_path)
            if _is_depth_feature(self.features[key]):
                video_info["video.is_depth_map"] = True
            self.info["features"][key]["info"] = {**existing_info, **video_info}
...
def _encode_depth_video_worker(
    video_key: str,
    episode_index: int,
    root: Path,
    fps: int,
    ...
    """_encode_video_worker()와 동일한 모듈-레벨(picklable) 워커 — ProcessPoolExecutor로
    RGB랑 같이 병렬 제출할 수 있게 depth 전용 codec/pix_fmt/extra_options로 인코딩."""
    temp_path = Path(tempfile.mkdtemp(dir=root)) / f"{video_key}_{episode_index:03d}.mp4"
    fpath = DEFAULT_IMAGE_PATH.format(image_key=video_key, episode_index=episode_index, frame_index=0)
    img_dir = (root / fpath).parent
    encode_video_frames(
        ...
        temp_path,
        fps,
        vcodec=feature_info.get("video.codec", "hevc"),
        pix_fmt=feature_info.get("video.pix_fmt", "gray12le"),
        crf=None,
        extra_options=feature_info.get("video.extra_options", {"x265-params": "lossless=1"}),
        overwrite=True,
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            video_path = self.root / self.meta.get_video_file_path(ep_idx, vid_key)
            feature = self.features[vid_key]
            if _is_depth_feature(feature):
                info = feature.get("info") or {}
                quantized = decode_depth_video_frames(
                    video_path,
                    shifted_query_ts,
                    self.tolerance_s,
                    pix_fmt=info.get("video.pix_fmt", "gray12le"),
                )
                frames = dequantize_depth(quantized, **_depth_quantizer_kwargs(feature))
            else:
    ...
    def add_frame(self, frame: dict) -> None:
        ...
        for key in frame:
            ...
            if self.features[key]["dtype"] == "video" and self._streaming_encoder is not None:
                ...
                image = frame[key]
                if _is_depth_feature(self.features[key]):
                    image = quantize_depth(image, **_depth_quantizer_kwargs(self.features[key]))
                compress_level = 1 if self.features[key]["dtype"] == "video" else 6
                self._save_image(image, img_path, compress_level)
    ...
    def save_episode(
        ...
        """
        _t_save_episode_start = time.perf_counter()
        ...
        _piper_log_timing = os.environ.get("PIPER_LOG_TIMING") == "1"
        _t0 = time.perf_counter()
        self._wait_image_writer()
        if _piper_log_timing:
            print(f"[PIPER_LOG_TIMING] _wait_image_writer(): {(time.perf_counter() - _t0) * 1e3:.1f}ms")
        ...
        if use_streaming:
            ...
            if _piper_log_timing:
                print(f"[PIPER_LOG_TIMING] compute_episode_stats(): {(time.perf_counter() - _t0) * 1e3:.1f}ms")

        _t0 = time.perf_counter()
        ep_metadata = self._save_episode_data(episode_buffer)
        if _piper_log_timing:
            print(f"[PIPER_LOG_TIMING] _save_episode_data(): {(time.perf_counter() - _t0) * 1e3:.1f}ms")
        ...
        if use_streaming:
            ...
            # depth는 hevc/gray12le/lossless 같은 자기만의 codec/pix_fmt가 필요해서 RGB용
            # _encode_video_worker(self.vcodec 하나만 적용)를 그대로는 못 쓰지만,
            # _encode_depth_video_worker로 같은 ProcessPoolExecutor 풀에 같이 제출해서
            # RGB+depth 전부 동시에 인코딩되게 함 — 순차로 하면 각 소요시간이 그대로
            # 더해짐(실측: RGB 병렬 17s + depth 순차 2개 21s+16s ≈ 총 54s이던 게,
            # 전부 동시면 이론상 가장 느린 것 하나 기준(~21s)까지 줄어듦).
            depth_keys = [k for k in self.meta.video_keys if _is_depth_feature(self.features[k])]
            rgb_keys = [k for k in self.meta.video_keys if k not in depth_keys]
            all_video_keys = rgb_keys + depth_keys
            _t_video = time.perf_counter()

            if parallel_encoding and len(all_video_keys) > 1:
                # TODO(Steven): Ideally we would like to control the number of threads per encoding such that:
                # num_cameras * num_threads = (total_cpu -1)
                # x265/SVT-AV1은 프로세스당 내부적으로 CPU 코어 수만큼 스레드를 잡으려 하므로,
                # 여러 개를 동시에 띄우면 스레드 수요가 코어 수를 훨씬 넘어서는 오버서브스크립션이
                # 됨 — self._encoder_threads를 명시적으로 안 정했으면 (총 코어 수 / 동시
                # 인코딩 개수)로 나눠서 프로세스당 스레드 수를 캡 씌움.
                encoder_threads = self._encoder_threads
                if encoder_threads is None:
                    cpu_count = os.cpu_count() or 1
                    encoder_threads = max(1, cpu_count // len(all_video_keys))

                with concurrent.futures.ProcessPoolExecutor(max_workers=len(all_video_keys)) as executor:
                    future_to_key = {}
                    ...
                    for video_key in depth_keys:
                        future = executor.submit(
                            _encode_depth_video_worker,
                            video_key,
                            episode_index,
                            self.root,
                            self.fps,
                            self.features[video_key].get("info") or {},
                            encoder_threads,
                        )
                        future_to_key[future] = video_key
            ...
            if _piper_log_timing:
                print(
                    f"[PIPER_LOG_TIMING] 영상 인코딩 전체({all_video_keys}): "
                    f"{(time.perf_counter() - _t_video) * 1e3:.1f}ms"
                )
        ...
        if _piper_log_timing:
            print(f"[PIPER_LOG_TIMING] meta.save_episode(): {(time.perf_counter() - _t0) * 1e3:.1f}ms")
            print(f"[PIPER_LOG_TIMING] save_episode() 전체: {(time.perf_counter() - _t_save_episode_start) * 1e3:.1f}ms")
    ...
    def _encode_temporary_episode_video(self, video_key: str, episode_index: int) -> Path:
        ...
        """
        feature = self.features[video_key]
        if not _is_depth_feature(feature):
            # RGB path: delegate to the shared worker so it respects the dataset-wide
            # vcodec/encoder_threads (incl. hardware encoders like nvenc/auto) — depth
            # always needs its own dedicated codec/pix_fmt below regardless of vcodec.
        ...
        encode_video_frames(
            ...
            temp_path,
            self.fps,
            vcodec=info.get("video.codec", "hevc"),
            pix_fmt=info.get("video.pix_fmt", "gray12le"),
            crf=None,
            extra_options=info.get(
