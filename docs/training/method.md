...
실측치는 `docs/training/smolvla_finetuning.md`(최초 60-episode 학습)와
`docs/training/new_smolvla_finetuning.md`(150-episode 재학습, 2026-08-08)에 있다.
...
한다. `scripts/tools/qc_studio.py`가 이 작업을 자동 제안 + 사람 확인으로 처리한다.

**GUI로 검수(권장, 사람이 직접 확인):**

```bash
python scripts/tools/qc_studio.py --folder records/<날짜>
```
...
```bash
python scripts/tools/qc_studio.py --folder records/<날짜> --report
```
...
```json
{
  ...
  "episodes": [
    {
      "source_dataset": "records/0802/erase_the_circle_0802-130542",
      "total_frames": 758,
      "enabled": true,
      "start_frame": 98,
      "end_frame": 607
...
| `--top-crop` / `--wrist-crop` | `x,y,size` 형식, 정사각형 크롭. 지금까지 전 데이터셋이 `280,0,720`(1280×720 원본에서 왼쪽 280px 제외)으로 통일돼 있다. 새 카메라 배치라면 `scripts/tools/preview_video_crop.py`로 미리 확인할 것. |
...
```bash
python scripts/tools/decode_check_parallel.py \
  local/<output_name> \
...
구현했다가 커밋하지 않아 유실됐다(`docs/training/new_smolvla_finetuning.md` 이전에 있던
...
전부 `docs/training/smolvla_finetuning.md`(최초 60-episode 학습)에서 검증된 값을
...
- `docs/training/smolvla_finetuning.md` — 최초 60-episode 학습(GOP≈200) 상세 기록, 설정값 선정 근거표
- `docs/training/new_smolvla_finetuning.md` — 150-episode 재학습(GOP=1) 진행 기록, 이번에 발견한 NVENC/GOP 버그와 실측 속도 비교
- `scripts/tools/prepare_erase_shape_dataset.py` — 데이터셋 빌드 스크립트 (`--help`로 전체 옵션 확인)
- `scripts/tools/qc_studio.py` — 프레임 범위 QC 도구
- `scripts/tools/decode_check_parallel.py` — 전수 디코딩 검증 도구(이번 세션에 추가)
