...
작성 2026-08-17. 2026-08-18에 실물 롤아웃으로 처음 돌렸고, 그 과정에서 나온 문제
(도형 오검출, GUI 한글 깨짐, AV1 미디코딩, 데이터셋 미마감, 파킹 지연, 컷오프가
녹화를 죽이는 문제 등)를 전부 수정·검증했다. **환경 세팅부터 명령까지 실전
절차는 [평가 세션 실행 설명서](eval_session_howto.md)를 볼 것** — 이 문서는
지표 정의와 근거를 다룬다.
...
**주관 배점은 주지 않는다.** "잘 접근했으니 0.7점" 같은 사람 판단 기반 부분
점수는 재현이 안 된다. 차등은 §2.1의 **물리 측정값**에만 둔다.
단계 간 배점은 "여기서 실패하면 뒤가 전부 불가능한가"로 가중했다.

#### 4단계(실질 완료)만 부분 점수 (2026-08-18 추가)

1·2·3·5단계는 0/1이지만, 4단계는 `erased_target`이 곧 물리 측정값이므로
비율만큼 부분 점수를 준다. 전부-아니면-전무로 두면 **84% 지운 시도와 30% 지운
시도가 똑같이 7/10**이 되어 정책 간 차이가 안 보인다(실물에서 실제로 겪음).

```text
erased ≤ 0.15        → 0            (개시선 미만은 3단계에서 이미 탈락)
0.15 < erased < 0.90 → 0.9 × (erased − 0.15) / (0.90 − 0.15) 배점
erased ≥ 0.90        → 만점 (3점)
```

완료선 아래 상한을 0.9로 둔 이유: 그냥 1까지 선형으로 이으면 `erased=0.899`가
반올림으로 10.00/10이 되면서 `success=False`인 만점이 나온다. **10/10은 실제
성공일 때만** 나와야 한다. 이 불연속은 완료선이 곧 성공 판정 기준이라는 사실을
반영한 것이다.

| `erased_target` | 총점(나머지 단계 통과 시) |
|---:|---:|
| 0.30 | 7.54 |
| 0.50 | 8.26 |
| 0.80 | 9.34 |
...
`/tmp/roi.png`에서 (a) 보드 영역, (b) 도형 bbox, (c) 나무 지우개 블록과 로봇 팔이
도형으로 잡히지 않았는지를 눈으로 본다. 안 맞으면 `EVAL_BOARD="x y w h"`로 덮어쓰고 값을 기록.

> **로봇 팔은 자동으로 걸러진다** (2026-08-18). 팔·그리퍼는 검은 무채색이라 채도 필터에
> 안 걸리고 면적·종횡비도 도형 조건을 통과해서, 예전에는 보드 오른쪽에 걸친 팔이 `circle`
> 도형으로 잡혔다. 지금은 "덩어리가 보드 오른쪽 경계에 닿으면 팔"(`BORDER_MARGIN`)로 가른다.
>
> **보드 폭을 줄여서 팔을 잘라내지 말 것.** 실측(6개 데이터셋)에서 팔은 보드 로컬 x=405부터
> 파고드는데 실제 도형은 x+w=465까지 뻗어 구간이 겹친다 — 폭으로 자르면 진짜 도형이 잘린다.
> 이 때문에 **도형은 그리퍼 쪽(보드 오른쪽 끝)에 그리지 않는다.**
