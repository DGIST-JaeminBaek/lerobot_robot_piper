...
```text
scripts/tasks/erase_shape/dataset/prepare_erase_shape_dataset.py
```
...
```text
scripts/piper/validation/preview_video_crop.py
```
...
```bash
python scripts/piper/validation/preview_video_crop.py \
...
실물 인간 승인 추론에서는
`scripts/piper/inference/piper_human_approved_inference.py`가 같은 `280,0,720` crop과
...
표준 mixed precision 패턴 그대로다. `pi0_finetuning.md`의 "BF16 정밀도"
