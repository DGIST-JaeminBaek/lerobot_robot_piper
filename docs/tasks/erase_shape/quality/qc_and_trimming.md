...
| [`qc_studio.py`](../../../../scripts/tasks/erase_shape/qc/qc_studio.py) | QC + 구간 확인 GUI, `--report`로 표만 출력 |
| [`qc_core.py`](../../../../scripts/tasks/erase_shape/qc/qc_core.py) | 판정 로직 (UI 없음) |
| [`episode_segmentation.py`](../../../../scripts/tasks/erase_shape/dataset/episode_segmentation.py) | 시작/종료 프레임 검출 |
| [`autofill_frame_ranges.py`](../../../../scripts/tasks/erase_shape/dataset/autofill_frame_ranges.py) | 기존 매니페스트 자동 채우기 / 마진 재적합 |
| [`export_cut_plan.py`](../../../../scripts/tasks/erase_shape/dataset/export_cut_plan.py) | JSON · CSV · ffmpeg 스크립트로 내보내기 |
| [`review_cuts.py`](../../../../scripts/tasks/erase_shape/qc/review_cuts.py) | 자른 지점 검토 시트 PNG 생성 |
...
{
  "format_version": 1,
  "generated_by": "scripts/tasks/erase_shape/qc/qc_studio.py",
...
```bash
python scripts/tasks/erase_shape/dataset/prepare_erase_shape_dataset.py --manifest configs/qc_review_local.json
```
...
```bash
python scripts/tasks/erase_shape/dataset/autofill_frame_ranges.py                    # 차이만 출력 (기본이 dry run)
python scripts/tasks/erase_shape/dataset/autofill_frame_ranges.py --only-missing --write   # 빈 칸만 채움
python scripts/tasks/erase_shape/dataset/autofill_frame_ranges.py --fit              # 기존 라벨로 마진 재적합
```
...
```bash
python review_cuts.py --all
python review_cuts.py --episode 0726-164325
```
...
```bash
python scripts/tasks/erase_shape/dataset/export_cut_plan.py --source-dir records/local --output tmp/cut_plans/local
```
