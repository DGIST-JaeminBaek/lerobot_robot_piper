...
배경은 docs/tasks/erase_shape/runtime/erase_run_design.md 참고. 요약:
...
sys.path.insert(0, str(SCRIPTS_DIR / "piper" / "inference"))
sys.path.insert(0, str(SCRIPTS_DIR / "tasks" / "erase_shape" / "evaluation"))
sys.path.insert(0, str(SCRIPTS_DIR / "tasks" / "erase_shape" / "lib"))
...
def judge_frame_from_video(record_path, expect_hw=(720, 1280)):
    ...
    러너가 이제 파킹 이동이 끝날 때까지 계속 녹화하므로(judge_frame_in_video),
    영상의 마지막 프레임이 곧 "팔이 다 빠진 최종 상태"다. 카메라에서 새로 한 장
    더 찍는 방식(grab())은 워밍업 때문에 몇 초가 걸리는데, 그 사이 사람이 이미
    다음 시도를 준비한다고 보드를 정리해버리는 일이 실물에서 반복됐다
    (2026-08-20 지적) — 이미 녹화된 마지막 프레임을 그대로 쓰면 그 지연이 없다.
    board ROI는 1280x720 원본 좌표계라 크롭본(512x512)으로 녹화한 경우엔 쓸 수
    없어 None을 돌려준다 — 그때는 호출부가 카메라에서 직접 찍는다.
    """
    if not record_path:
        return None
    try:
...
def run_attempt_with_runner(settings, on_log=None, on_step=None,
    ...
    return {
        ...
        # 파킹 이어찍기의 마지막 원본 프레임(RGB, 압축 전). 영상에서 디코딩한
        # 프레임보다 이걸 우선 쓴다 — 압축 편향은 piper_infer_runner의
        # judge_frame_raw 주석 참고. 없으면(파킹 녹화를 못 했거나 크롭본) None.
        "_judge_frame_raw": run.judge_frame_raw,
        "_steps": steps,
...
def save_step_traces(attempts: list[dict], out_path: Path) -> Path | None:
    ...
    for entry in attempts:
        # 판정 프레임(numpy 배열)은 main()이 이미 pop해서 쓰지만, 경로가 바뀌어도
        # log.json 직렬화가 깨지지 않게 여기서 한 번 더 확실히 털어낸다.
        entry.pop("_judge_frame_raw", None)
...
def main():
    ...
    p.add_argument(
        ...
        help="HAMLET 등 외부 policy 패키지명 (예: smolvla_hamlet). "
             "해당 패키지는 PYTHONPATH에서 import 가능해야 한다",
    )
    ...
    # 940(시연 중앙값 720의 약 1.3배) -> 1410(약 2배) -> 2100(약 2.9배)로 올렸다.
    # 실물 2026-08-18에 940스텝을 다 쓰고도 마무리를 못 해 cutoff로 끝나는 시도가
    # 반복됐다(예: 84.4% 지운 채 종료 — 꼭짓점 잔여). 시간을 더 주고 스스로
    # 끝내는지 본다. --stop-on-release가 켜져 있으면 다 놓는 즉시 끊기므로
    # 상한을 올려도 성공하는 시도가 길어지지는 않는다.
    p.add_argument("--max-steps", type=int, default=2100,
                   help="시도당 상한 (시연 중앙값 720의 약 2.9배). 예전 기본값은 940, 1410")
    ...
    p.add_argument("--vcodec", default=os.environ.get("VCODEC", "h264"),
                   help="녹화 영상 인코더. 기본은 configs/recording.env의 VCODEC(teleop 녹화와 "
                        "동일 — 보통 hevc_nvenc), 없으면 h264. hevc/h264는 둘 다 cv2로 바로 "
                        "읽힌다(ink_metric.CV2_CODECS) — libsvtav1(AV1)만 세그폴트 위험 "
                        "때문에 피할 것(2026-08-18)")
    ...
    p.add_argument("--rviz", action="store_true",
                   help="예측 action을 /joint_states에 publish한다 (RvizPublisher). "
                        "ROS2를 미리 source하고 RViz/robot_state_publisher를 띄워둘 것 "
                        "(docs/rviz_setup.md). 기본은 꺼짐 — 안 켜면 기존 동작과 동일")
    p.add_argument("--mode", default="demo", help="러너 모드 프리셋 (demo | augment)")
    p.add_argument(
        "--top-crop",
        default="280,0,720",
        help="X,Y,SIZE. 학습 데이터를 만든 값과 같아야 한다 "
             "(docs/tasks/erase_shape/training/smolvla_finetuning.md 기준 280,0,720)",
    )
    ...
    # 재추론 시점. 기본값(None)이면 러너 기본을 그대로 쓴다 — 지금까지 검증된 거동을
    # 조용히 바꾸지 않기 위해서다(threshold 0.3 = 35스텝마다).
    #
    # HAMLET에서 이 값이 중요한 이유: memory_stride는 "관측 히스토리를 몇 프레임
    # 간격으로 보는가"인데, 재추론 간격을 거기 맞추면 메모리 슬롯 4개가 직전 4번의
    # 추론 시점과 1:1로 맞물린다. threshold 방식에서 소비되는 스텝은
    #     horizon - horizon*threshold = horizon*(1 - threshold)
    # 이므로, stride에 맞추려면
    #     chunk_threshold = (horizon - stride) / horizon
    # 예) horizon 50: stride 35 -> 0.3(기존 기본값), stride 16 -> 0.68
    p.add_argument("--chunk-threshold", type=float, default=None,
                   help="threshold 트리거에서 남은 큐 비율이 이 값 이하로 떨어지면 재추론. "
                        "HAMLET은 (horizon - memory_stride)/horizon으로 맞추면 메모리 "
                        "슬롯과 추론 시점이 정렬된다 (horizon 50 기준 stride16 -> 0.68). "
                        "생략하면 러너 기본값(0.3)")
    p.add_argument("--trigger-mode", choices=("threshold", "fixed"), default=None,
                   help="재추론 트리거 방식. 생략하면 러너 기본값(threshold)")
    p.add_argument("--aggregate-fn", default=None,
                   ...
                   help="chunk 합치는 방식. 기본은 러너 프리셋 값 "
                        ...
                        "docs/tasks/erase_shape/runtime/erase_run_design.md §5.5 참고")
    ...
    # 트리거 설정은 RunSettings 최상위라 smoothing과 달리 그냥 얹으면 된다.
    if args.trigger_mode:
        base["trigger_mode"] = args.trigger_mode
    if args.chunk_threshold is not None:
        if not 0.0 < args.chunk_threshold < 1.0:
            raise SystemExit(f"--chunk-threshold는 0과 1 사이여야 한다: {args.chunk_threshold}")
        base["chunk_threshold"] = args.chunk_threshold
        horizon = 50  # RunSettings.horizon 기본값. 아래는 사람이 읽는 안내일 뿐이다.
        every = horizon * (1.0 - args.chunk_threshold)
        print(f"[INFO] chunk_threshold = {args.chunk_threshold:g} "
              f"→ 약 {every:.0f}스텝마다 재추론 (30Hz 기준 {every/30:.2f}초), "
              f"겹치는 chunk 약 {horizon/max(every,1e-9):.1f}개")
    ...
    (run_dir / "meta.json").write_text(json.dumps({
        ...
        # 재현에 필요하다 — 0823 세션을 나중에 재구성할 때 트리거 설정이 안 남아 있어
        # meta.json만으로는 같은 조건을 못 만들었다.
    ...
    status = None if args.no_status else ES.LiveStatus(
        max_attempts=1, max_steps=args.max_steps, hil=args.hil
    )
    ...
    if use_panel:
        ...
        panel = HilPanel(1, args.max_steps).start()
    ...
    try:
        ...
        # 재시도 없음 — 시도 1회. release나 max-steps까지 VLA가 스스로 제어하게 두고,
        # 그 결과를 그대로 판정한다.
        i = 1
        if status:
        ...
        # 판정 프레임은 녹화 영상의 마지막 프레임을 그대로 쓴다. 러너가 파킹
        # 이동이 끝날 때까지 계속 녹화하므로(judge_frame_in_video) 그 마지막
        # 프레임이 곧 "다 지우고 팔도 빠진 최종 상태"다. 카메라에서 새로 한
        # 장 더 찍는 grab()은 워밍업 때문에 몇 초 늦게 찍혀서, 그 사이 사람이
        # 다음 시도를 준비한다고 보드를 이미 정리해버리는 일이 실물에서
        # 반복됐다(2026-08-20 지적) — 그래서 grab()은 영상을 못 쓸 때만 쓴다.
        record_path = summary.get("record_path")
        # ★ 우선순위 1: 파킹 이어찍기의 마지막 **압축 전 원본** 프레임.
        # 영상에서 디코딩한 프레임은 crf/qp=30으로 뭉개진 뒤라, 무손실인 기준
        # 프레임과 화질이 비대칭이다. 실측(2026-08-21): 같은 사진을 h264 crf30에
        # 통과시키면 잉크 비율이 6.4% 줄어 ink_after만 과소평가되고 erased_frac이
        # 성공 쪽으로 계통 편향된다(90% 임계 근처에서 +0.6~2%p). 원본을 쓰면
        # 기준/판정 두 프레임이 같은 무손실 조건이 되어 그 편향이 사라진다.
        # 러너는 RGB로 주는데 판정기(ink_metric)는 BGR 기준이라 변환한다.
        # numpy 배열이 log.json으로 새면 직렬화가 깨지므로 여기서 pop한다.
        raw_judge = summary.pop("_judge_frame_raw", None)
        after_frame = None
        if raw_judge is not None:
            import cv2

            candidate = cv2.cvtColor(np.asarray(raw_judge), cv2.COLOR_RGB2BGR)
            if candidate.shape[:2] == (720, 1280):
        ...
        if summary["status"] not in ("finished", "hil_abort"):
            print(f"[STOP] 러너가 {summary['status']}로 끝났다.")
    finally:
        if panel:
            panel.close()
