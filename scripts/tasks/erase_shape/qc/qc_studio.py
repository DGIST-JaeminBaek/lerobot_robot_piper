...
"""Review a folder of recordings and confirm each episode's trim range.
    ...
    python qc_studio.py                          # UI on records/local
    python qc_studio.py --folder records/0727/erase_the_circle
    python qc_studio.py --report                 # no window, print the table
...
LEGACY_GENERATED_BY = "scripts/tools/qc_studio.py"
...
class Studio(tk.Tk):
    ...
    def save(self) -> None:
        ...
        if self.output.exists():
            ...
            if existing.get("generated_by") not in {GENERATED_BY, LEGACY_GENERATED_BY}:
