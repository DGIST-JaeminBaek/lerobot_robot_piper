...
class EraseChecker:
    ...
    def set_reference(self, frame, exclude=None):
        """시도 전 프레임에서 도형을 검출하고 각 도형의 잉크량을 기록한다.

        exclude: [(x,y,w,h), ...] 전역좌표. 보드에 고정된 이물질(테이프 자국 등)이
        도형으로 오검출되는 걸 막는다 — ink_metric.detect_shapes 참고.
        """
        detected, white = M.detect_shapes(frame, self.board, self.dark_ratio, exclude)
        if not detected:
    ...
    def check(self, frame, target):
        ...
        # (docs/tasks/erase_shape/runtime/erase_run_design.md §4.6-③: 전달할 통로가 없다).
        residual = None
...
def first_last_frames(video):
    """영상 첫/마지막 프레임. cv2.VideoCapture를 직접 열지 않고
    ink_metric.read_frames()를 쓴다 — 그게 코덱을 먼저 확인해서 cv2가 못 읽는
    AV1(libsvtav1) 등은 PyAV로 우회한다. cv2에 AV1을 그냥 넘기면 디코더를
    못 찾을 뿐 아니라 간헐적으로 프로세스가 죽는다(2026-08-18 실물: 평가 GUI가
    Segmentation fault로 죽음)."""
    first = last = None
