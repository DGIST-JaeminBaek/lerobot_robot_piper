...
그래서 `judge.py`는 task 런타임·게이트 모듈을 import하지 않는다. 도형 찾기,
...
이 도구는 `erase_run.py`가 이미 남기는 산출물만 읽는 사후(offline) 처리다.
`erase_run.py`, `erase_check.py`, `erase_stats.py`, `13__erase_gate.sh`의 실행 경로를
바꾸지 않으며 로봇·카메라·정책 연결도 필요 없다.

**지우는 법:**

```bash
rm -rf scripts/tasks/erase_shape/evaluation/eval_kit
...
```bash
python scripts/tasks/erase_shape/evaluation/eval_kit/stamp.py --latest records/hil \
...
```bash
python scripts/tasks/erase_shape/evaluation/eval_kit/summarize.py --markdown
```
...
```bash
python scripts/tasks/erase_shape/evaluation/eval_kit/calibrate.py --make-sheet 40 --runs 'records/hil/*'
# 3명이 따로 채운다 (rater_A/B/C에 1 또는 0)
python scripts/tasks/erase_shape/evaluation/eval_kit/calibrate.py --analyze outputs/analysis/labels.csv
```
