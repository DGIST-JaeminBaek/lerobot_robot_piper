...
def write_json(rows: list[dict[str, Any]], path: Path, task: str) -> None:
    document = {
        "format_version": 1,
        "target_task": task,
        "generated_by": "scripts/tasks/erase_shape/dataset/export_cut_plan.py",
...
def write_shell(rows: list[dict[str, Any]], path: Path) -> None:
    lines = [
        "#!/usr/bin/env bash",
        "# Frame-exact video cuts. Generated by scripts/tasks/erase_shape/dataset/export_cut_plan.py -- regenerate, do not edit.",
        "set -euo pipefail",
