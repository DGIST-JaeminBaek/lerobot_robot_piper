...
import numpy as np
from effort_common import effort_schema, load_info, parquet_files, read_state_table
...
def main(root_str: str) -> int:
    ...
    info = load_info(root)
    schema = effort_schema(info)
    if schema is None:
        print(f"{BAD} observation.state 피처가 없음{END}")
        return 1

    names = schema.names
    eff_i = schema.effort_indices
    vel_i = schema.velocity_indices
    pos_i = schema.position_indices
    ...
    if not files:
        ...
        return 1
    state, ep, fi = read_state_table(files[0])
