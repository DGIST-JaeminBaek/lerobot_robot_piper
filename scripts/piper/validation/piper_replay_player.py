...
from rviz_joint_state import (  # noqa: E402
    PIPER_GRIPPER_NAME,
    PIPER_JOINT_NAMES,
    piper_normalized_to_physical,
)
...
class _RvizPublisher:
    """ROS2 의존성을 --rviz 요청 때만 여는 JointState publisher."""
    ...
    def __init__(self, topic: str) -> None:
        try:
            ...
            raise RuntimeError(
                "--rviz는 ROS2 환경이 필요합니다. source /opt/ros/humble/setup.bash 후 실행하세요."
            ) from exc
        ...
        class ReplayNode(Node):
            def __init__(self) -> None:
                super().__init__("piper_replay_player")
                self.publisher = self.create_publisher(JointState, topic, 10)

        self._node = ReplayNode()
        self._topic = topic

    def publish(self, normalized_action: list[float]) -> None:
        msg = self._joint_state_type()
        msg.header.stamp = self._node.get_clock().now().to_msg()
        msg.name = PIPER_JOINT_NAMES + [PIPER_GRIPPER_NAME]
        msg.position = [
            piper_normalized_to_physical(name, value)
            for name, value in zip(msg.name, normalized_action, strict=True)
        ]
...
def _load_video_frames(
    ...
    부담 없음. 영상 전체를 메모리에 두므로 Frame 슬라이더의 즉시 탐색도 가능하다.
...
def play(initial_args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        rviz_key = args.rviz_key
        if args.rviz:
            if rviz_key not in df.columns:
                raise ValueError(f"--rviz-key '{rviz_key}' 컬럼이 없습니다")
            invalid = [
                index for index, value in enumerate(df[rviz_key])
                if len(_as_float_list(value)) != 7
            ]
            if invalid:
                raise ValueError(
                    f"--rviz-key '{rviz_key}'는 Piper 7축 정규화 action이어야 합니다 "
                    f"(frame {invalid[0]}의 길이: {len(_as_float_list(df[rviz_key].iloc[invalid[0]]))})"
                )
        ...
        frame_pos = min(max(0, args.start_frame), total_frames - 1)
        ...
        rviz_publisher = _RvizPublisher(args.joint_state_topic) if args.rviz else None
        slider_request: list[int | None] = [None]
        slider_updating = [False]

        cv2.namedWindow(args.window_name, cv2.WINDOW_NORMAL)

        def request_seek(position: int) -> None:
            if not slider_updating[0]:
                slider_request[0] = position

        cv2.createTrackbar("Frame", args.window_name, frame_pos, total_frames - 1, request_seek)
        ...
        try:
            while 0 <= frame_pos < total_frames and episode_running:
                if slider_request[0] is not None:
                    frame_pos = slider_request[0]
                    slider_request[0] = None
                    paused = True

                if rviz_publisher is not None:
                    rviz_publisher.publish(_as_float_list(df.iloc[frame_pos][rviz_key]))
            ...
            # 에피소드 정보를 패널에 덮어쓰기
                ...
                cv2.setTrackbarPos("Frame", args.window_name, frame_pos)
...
""".strip(),
    ...
    parser.add_argument("--rviz", action="store_true", help="현재 프레임의 Piper action을 ROS2 JointState로 publish")
    parser.add_argument("--joint-state-topic", default="/joint_states", help="--rviz JointState publish topic")
    parser.add_argument("--rviz-key", default="action", help="--rviz에 publish할 7축 정규화 action 컬럼")
