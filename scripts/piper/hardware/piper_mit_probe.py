...
사용 예 (권장 순서):
    ...
    python scripts/piper/hardware/piper_mit_probe.py --joint 1 --kp 5 --hold-s 3

    # 2) 괜찮으면 게인을 올려가며
    python scripts/piper/hardware/piper_mit_probe.py --joint 1 --kp 10
    python scripts/piper/hardware/piper_mit_probe.py --joint 1 --kp 20

    # 3) 작은 왕복으로 추종 확인 (정규화 단위 진폭)
    python scripts/piper/hardware/piper_mit_probe.py --joint 1 --kp 10 --amplitude 3 --cycles 2
    ...
    try:
        ...
        try:
            if args.park:
                ...
                print(
                    "        해제하려면: python scripts/piper/hardware/safe_release_torque.py"
                )
        except Exception as error:
