...
사용법:
    python scripts/piper/hardware/kinematics_check.py --port can_follower
    python scripts/piper/hardware/kinematics_check.py --port can_follower --samples 5 --interval 1.0

옵션:
    --port      follower CAN 인터페이스 (기본 can_follower)
...
def main() -> None:
    ...
    p.add_argument("--port", default="can_follower", help="follower CAN 인터페이스")
