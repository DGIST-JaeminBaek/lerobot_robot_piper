...
공용 추론 러너(piper_infer_runner.py)와 완전히 같은 실행 경로를 쓰되 RViz/로봇 없이
...
from piper_infer_runner import Event, InferenceRunner, RunSettings, load_env_file  # noqa: E402
...
def run_one(base: dict, smoothing: SmoothingConfig, label: str) -> tuple[np.ndarray, list[str]]:
    ...
    """
    settings = RunSettings.from_mode("demo", **base, smoothing=smoothing)
    events: queue.Queue = queue.Queue()
    worker = InferenceRunner(settings, events)
    ...
    while status is None:
        try:
            ...
            raise RuntimeError("runner timed out")
        if kind == Event.LOG:
