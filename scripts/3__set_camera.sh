...
if [[ -z "${TOP}" || -z "${WRIST}" ]]; then
  echo "Usage: bash scripts/3__set_camera.sh TOP_CAM WRIST_CAM"
  echo "Example: bash scripts/3__set_camera.sh 6 0"
  exit 2
fi

if [[ ! -f "${ENV_FILE}" ]]; then
  cp "${REPO_DIR}/configs/recording.env.example" "${ENV_FILE}"
fi
