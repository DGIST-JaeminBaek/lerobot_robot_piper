...
실행: python -m pytest scripts/tests/piper/inference/test_infer_runner_mock.py
"""
...
class FakeLeRobotDataset:
    ...
    def __init__(self, **kwargs):
        self.kwargs = kwargs
        self.vcodec = kwargs.get("vcodec", "h264")
...
def test_worker_carries_immutable_hamlet_history_snapshot():
    run = _new_runner()
    history = object()
    seen = []

    def predict(observation, snapshot):
        seen.append((observation["v"], snapshot))
        return np.zeros((5, 7), np.float32)

    run._start_inference_worker(predict)
    run._request_inference({"v": 3.0}, step=0, rollout_history=history)
    assert run._await_chunk(timeout=5.0, step=0, align=False) is not None
    assert seen == [(3.0, history)]
    run.stop_event.set()
